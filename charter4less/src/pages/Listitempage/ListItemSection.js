...
const schema = z.object({
  itemName: z
    .string()
    .max(50, "Item name must not be more than 50 characters")
    .min(5, "Item name must be at least 5 characters"),
  itemDescription: z
    .string()
    .max(2000, "Item description must not be more than 2000 characters")
    .min(20, "Item description must be at least 20 characters"),
  itemPrice: z
    .string()
    .regex(/^[1-9]+\.?\d*$/, "Price must be a positive number greater than 0"),
  itemLocation: z
    .string()
    .max(50, "Please enter a valid location")
  ...
  itemImage: z
    ...
    .refine((file) => file && file.length > 0, {
      message: "Upload at least one image of the item",
    })
    .refine((file) => file && file.length <= 5, {
...
export default function ListItemSection() {
  ...
  const {
    ...
    setItemData,
    uploadError,
  } = useAddItem({ reset: reset });
  const { setSessionData } = useSessionStore();

  // update session data after successful form submission
  useEffect(() => {
    if (docAdded) {
      setSessionData(null);
    }
  }, [docAdded, setSessionData]);
        ...
        <div className="form-wrapper">
          {docAdded ? (
            ...
            <Alert variant="danger" dismissible>
              Something went wrong while listing item. Try again later
            </Alert>
          ) : (
            ""
          )}
                ...
                <Row className="form-sections-row2">
                  <Col>
                    <Form.Group controlId="">
                      ...
                      <Form.Control
                        isInvalid={errors.itemImage}
                        {...register("itemImage", {
                          required: "Upload at least one image of the item",
